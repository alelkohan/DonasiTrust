...
class ProfileController extends Controller
{
    ...
    public function submitVerification(Request $request, AuditLogger $audit, TotpGuard $totp)
    {
        $user = $request->user();
         ...
         */
        if ($gantiRekening && $user->hasTwoFactorEnabled()) {
            $totp->assertValid($user, $request->input('totp_code'), 'profile.payout_account_changed');
        }

        // Satu NIK hanya boleh dipakai satu akun. Dicek lewat HMAC, sehingga
        // duplikat bisa dideteksi tanpa menyimpan NIK-nya di basis data.
        ...
        // Disk "local" berakar di storage/app/private — tidak bisa diakses lewat URL.
        // Berkas hanya bisa dibuka lewat route terproteksi milik admin.
        ...
        $user->update([
            ...
            // Nomor lengkap TIDAK disimpan — hanya 4 digit terakhir untuk
            // pencocokan visual admin, plus HMAC untuk mendeteksi duplikat.
            'identity_number_last4' => substr($data['identity_number'], -4),
            'identity_number_hash' => $nikHash,
            'bank_name' => $data['bank_name'],
            'bank_account_number' => $rekeningBaru,
        ...
        if ($gantiRekening) {
            $audit->record('user.payout_account_changed', $user, [
                'rekening_baru' => $user->fresh()->maskedPayoutAccount(),
            ...
            $user->notify(new PayoutAccountChanged($user->fresh()->maskedPayoutAccount()));
        }
